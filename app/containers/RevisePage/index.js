...
export class RevisePage extends React.PureComponent {
  constructor(props) {
    super(props);
    this.state = {
      modal: {},
  ...
  render() {
    const {loading, detail, examResult} = this.props;
        ...
        {/************************************第四步*******************************/}
        {
          ...
          <Section>
            <ExamInfo detail={detail} type={3}>
              <ExamResult>
                <div>
                  <ExamScore>{examResult.revisescore}</ExamScore>分
                </div>
...
const withReducer = injectReducer({key: 'revise', reducer});
const withSaga = injectSaga({key: 'revise', saga});
