...
import { GET_OPENID, GET_USER_INFO, UPDATE_USER_INFO } from './constants';
import { getOpenidSuccess, getOpenidError, getUserInfoSuccess, getUserInfoError, updateUserInfoSuccess, updateUserInfoError  } from './actions';
...
export function* updateUserInfoSaga() {
  while (true) {
    const { payload } = yield take(UPDATE_USER_INFO);
    const { response, error } = yield call(Api.updateUserInfo, payload);
    if (response && !error) {
